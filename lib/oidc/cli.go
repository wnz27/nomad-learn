...
type CallbackServer struct {
	...
	nonce     string
	errCh     chan error
	successCh chan *structs.OIDCCallbackRequest
}
...
func NewCallbackServer() (*CallbackServer, error) {
	...
	ln, err := net.Listen("tcp", "127.0.0.1:51835")
	if err != nil {
		return nil, nil
	}
	...
	srv := &CallbackServer{
		...
		successCh: make(chan *structs.OIDCCallbackRequest, 5),
	}
...
func (s *CallbackServer) SuccessCh() <-chan *structs.OIDCCallbackRequest {
	return s.successCh
}
...
func (s *CallbackServer) ServeHTTP(w http.ResponseWriter, req *http.Request) {
	...
	cbParams := &structs.OIDCCallbackParams{
		AuthMethod:  "Auth0",
		State:       q.Get("state"),
		Code:        q.Get("code"),
		ClientNonce: s.nonce,
	...
	// Build our result
	result := &structs.OIDCCallbackRequest{
		Auth: cbParams,
	}
