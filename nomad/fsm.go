...
func (n *nomadFSM) Apply(log *raft.Log) interface{} {
	...
	case structs.ACLRolesDeleteByNameRequestType:
		return n.applyACLRolesDeleteByName(msgType, buf[1:], log.Index)
	}
...
func (n *nomadFSM) applyACLRolesDeleteByName(msgType structs.MessageType, buf []byte, index uint64) interface{} {
	defer metrics.MeasureSince([]string{"nomad", "fsm", "apply_acl_role_delete_by_name"}, time.Now())
	var req structs.ACLRolesDeleteByNameRequest
	if err := structs.Decode(buf, &req); err != nil {
		panic(fmt.Errorf("failed to decode request: %v", err))
	}

	if err := n.state.DeleteACLRolesByName(msgType, index, req.ACLRoleNames); err != nil {
		n.logger.Error("DeleteACLRolesByName failed", "error", err)
		return err
	}
