...
func TestStateStore_UpsertACLRoles(t *testing.T) {
	...
	// Generate a mocked ACL role for testing and upsert this straight into
	// state.
	...
	for raw := iter.Next(); raw != nil; raw = iter.Next() {
		...
		must.Eq(t, 10, aclRole.CreateIndex)
		must.Eq(t, 10, aclRole.ModifyIndex)
	}
	...
	updatedMockedACLRole.Policies = []string{"mocked-test-policy-1"}
	updatedMockedACLRole.SetHash()
	require.NoError(t, testState.UpsertACLRoles(
		structs.MsgTypeTestSetup, 20, []*structs.ACLRole{updatedMockedACLRole}))
...
func TestStateStore_DeleteACLRolesByName(t *testing.T) {
	ci.Parallel(t)
	testState := testStateStore(t)
	...
	err := testState.DeleteACLRolesByName(structs.MsgTypeTestSetup, 20, []string{"not-a-role"})
	...
	err = testState.DeleteACLRolesByName(structs.MsgTypeTestSetup, 20, []string{mockedACLRoles[0].Name})
	require.NoError(t, err)
	...
	// table index.
	err = testState.DeleteACLRolesByName(structs.MsgTypeTestSetup, 30, []string{mockedACLRoles[1].Name})
	require.NoError(t, err)
