	...
	testCases := []struct {
	...
	}{
		{
			...
			inputACLRole: &ACLRole{
				...
				Policies:    []string{"mocked-test-policy-1", "mocked-test-policy-2"},
				CreateIndex: 10,
				ModifyIndex: 10,
				Hash:        []byte{},
			},
	...
	testCases := []struct {
		...
		},
		{
			...
			inputACLRole: &ACLRole{
				Name:        "acl-role",
				Description: "mocked-test-acl-role",
				Policies:    []string{"mocked-test-policy-1"},
				CreateIndex: 10,
				ModifyIndex: 10,
				Hash: []byte{
					137, 147, 2, 29, 53, 94, 78, 13, 45, 51, 127, 193, 21, 248, 230, 126, 34,
...
func Test_ACLRolesDeleteByNameRequest(t *testing.T) {
	req := ACLRolesDeleteByNameRequest{}
	require.False(t, req.IsRead())
}
