...
type ACLRole struct {
	...
	// supplied by the operator on role creation. There is no UUID as operators
	// need the ability to lookup and use roles in a human friendly manner.
	...
	// Policies is an array of ACL policy names. This array should have at
	// least one entry.
	Policies []string
...
// ACLRolesDeleteByNameRequest is the request object to delete one or more ACL
// roles using the role name.
type ACLRolesDeleteByNameRequest struct {
	ACLRoleNames []string
	WriteRequest
}

// ACLRolesDeleteByNameResponse is the response object when performing a
// deletion of one or more ACL roles using the role name.
type ACLRolesDeleteByNameResponse struct {
	WriteMeta
}
