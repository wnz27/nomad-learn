...
func (e *Eval) deleteEvalsByFilter(args *structs.EvalDeleteRequest) (int, uint64, error) {
	...
	// but for large sets we don't want recreate our paginator. For the intended
	// use case it's ok if we miss a handful of evals if they're inserted behind
	// the paginator's cursor.
	snap, err := e.srv.State().Snapshot()
	if err != nil {
